...
public class SparkMapFilter {

    public static void main(String[] args) {
        // configure spark
        SparkConf sparkConf = new SparkConf()
                .setAppName("Spark Map from CSV to elastic")
        ...
        JavaRDD<Map<String, Object>> rdd_data = rdd_raw_data.map(x -> {
            String[] test = x.split(",(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)");
            ...
            try {
                if (test[0].length() == 25) {
                    try {
                        ...
                        test[0] = om.writeValueAsString(timeInMillis);
                        final_result.put("created_at",Long.parseLong(test[0]));
        ...
        JavaEsSpark.saveToEs(rdd_data,"spark-index/twitter-spark");
    }
}
